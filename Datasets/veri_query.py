...
class VeRi(BaseImageDataset):
    ...
    def __init__(self, root='datasets', verbose=True, **kwargs):
        ...
        # train = self.process_dir(self.train_dir, self.train_list, relabel=True)  #Purpose Comment
        ...
        # self.train = train     #Purpose Commnet
        self.query = query      #Purpose Comment
        # self.gallery = gallery  #Purpose Comment

        # self.num_train_pids, self.num_train_imgs, self.num_train_cams = self.get_imagedata_info(self.train)  #Purpose Comment
        self.num_query_pids, self.num_query_imgs, self.num_query_cams = self.get_imagedata_info(self.query)   #Purpose Comment
        # self.num_gallery_pids, self.num_gallery_imgs, self.num_gallery_cams = self.get_imagedata_info(self.gallery)  #Purpose Comment
    ...
    def check_before_run(self):
        ...
        if not osp.exists(self.query_dir):  #Purpose Comment
            raise RuntimeError('"{}" is not available'.format(self.query_dir)) #Purpose Comment
        # if not osp.exists(self.gallery_dir): #Purpose Comment
        #     raise RuntimeError('"{}" is not available'.format(self.gallery_dir)) #Purpose Comment
...
V = VeRi()
dir_path = '/home/rutu/WPI/Directed_Research/ReID_Datasets/VeRi'
V.process_dir(dir_path, list_path=None, relabel=False)
